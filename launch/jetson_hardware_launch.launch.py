...
def generate_launch_description():
    ...
    depthai_resources_path = os.path.join(depthai_examples_share, 'resources')
    ...
    detection_arg = DeclareLaunchArgument(
        'detection',
        default_value='true',
        description='Launch OAK-D YOLOv4 spatial detection and visualisation.'
    )
    ...
    serial_bridge = Node(
        ...
        output='screen',
        parameters=[
            os.path.join(ursula_share, 'config', 'serial_bridge_params.yaml')
        ]
    )
    ...
    twist_mux = Node(
        ...
        output='screen',
        parameters=[
            os.path.join(ursula_share, 'config', 'twist_mux.yaml')
        ],
        remappings=[
            ('/cmd_vel_out', '/cmd_vel')
        ]
    ...
    oak_tracker = Node(
        ...
        parameters=[{
            'tf_prefix': 'oak',
            'monoResolution': '400p',
            'resourceBaseFolder': depthai_resources_path,
